...
function local_sync_cert_get_errors() {
    ...
    foreach ($data_fail as $data) {
        $issues = $DB->get_record_sql('SELECT * FROM {local_sync_cert_send_log} WHERE log_id = ' . $data->id . ' AND status = 0 ORDER BY timecreated DESC LIMIT 1');
...
function local_sync_cert_cron() {
    ...
    local_sync_cert_log_api_call_summary($success_count, $error_count);
}

function local_sync_cert_log_api_call_summary($success_count, $error_count){
    global $DB;
    ...
    return $DB->insert_record('local_sync_cert_api_calls', $data);
}
...
function local_sync_response_update($employee_identifier, $document_doc, $status, $response) {
    ...
    $course = $DB->get_records_sql("SELECT cd.id, cf.shortname, cd.value, cd.instanceid FROM {customfield_field} cf INNER JOIN {customfield_data} cd ON cd.fieldid = cf.id WHERE cf.shortname = 'doc_code' AND cd.value = '" . $document_doc . "' ");
    $tmp_courses = 0;
...
function local_sync_cert_get_contractor($courseid) {
    global $DB;
    $sql_users = "SELECT {user}.id, {user}.email, {user}.username FROM {course} INNER JOIN {context} ON {context}.instanceid = {course}.id INNER JOIN {role_assignments} ON {context}.id = {role_assignments}.contextid INNER JOIN {role} ON {role}.id = {role_assignments}.roleid INNER JOIN {user} ON {user}.id = {role_assignments}.userid WHERE {role}.id = 3 AND {course}.id = " . $courseid;
...
function local_sync_cert_get_obs($course, $user) {
    global $DB;
    $grade = $DB->get_record_sql("SELECT ROUND(gg.finalgrade) as 'grade' FROM {grade_grades} gg INNER JOIN {grade_items} gi ON gg.itemid = gi.id WHERE gi.itemtype = 'course' AND gg.userid = " . $user->id . ' AND gi.courseid = ' . $course->id);
...
function local_sync_cert_get_customfield($name, $courseid) {
    global $DB;
    return $DB->get_record_sql("SELECT cf.shortname, cd.value FROM {customfield_field} cf INNER JOIN {customfield_data} cd ON cd.fieldid = cf.id WHERE cf.shortname = '" . $name . "' AND cd.instanceid = " . $courseid);
}
